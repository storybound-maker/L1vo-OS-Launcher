...
fun AppHub(
    ...
    fun openL1voLabel(label: String) {
        when (label) {
            "LEAU" -> onLeau()
        ...
        if (query.isBlank()) {
            item(span = { GridItemSpan(maxLineSpan) }) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    L1voAppsPanel(Modifier.weight(1f), ink) { openL1voLabel(it) }
                    SystemAppsPanel(
                        Modifier.weight(1f), ink,
                        onSettings = { launch(context, Intent(Settings.ACTION_SETTINGS)) },
                        onPhone = { launch(context, Intent(Intent.ACTION_DIAL)) },
                        onMessages = { launchMessages(context) },
            ...
            items(8) { index ->
                ...
                FavoriteSlot(app, ink, if (app != null) {{ openApp(app) }} else null)
            }
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
                Surface(onClick = onLeau, shape = RoundedCornerShape(50), color = L1voInk, shadowElevation = 6.dp) {
                    Row(Modifier.padding(horizontal = 20.dp, vertical = 12.dp), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Text("LEAU", color = Color.White, fontWeight = FontWeight.Bold)
                    }
                }
            }
        }
...
private fun SearchHub(query: String, onQueryChange: (String) -> Unit, ink: Color) {
    Surface(shape = RoundedCornerShape(50), color = Color.White.copy(alpha = .90f), shadowElevation = 4.dp, modifier = Modifier.fillMaxWidth().height(56.dp)) {
        Row(Modifier.fillMaxSize().padding(horizontal = 18.dp), verticalAlignment = Alignment.CenterVertically) {
            Icon(Icons.Outlined.Search, "Search Hub", tint = L1voDeepGreen, modifier = Modifier.size(24.dp))
            ...
            BasicTextField(
                ...
                decorationBox = { inner ->
                    if (query.isEmpty()) Text("SEARCH HUB", color = ink.copy(alpha = .72f), fontWeight = FontWeight.SemiBold)
                    inner()
                }
            )
            if (query.isNotEmpty()) {
...
private fun SystemAppsPanel(
    ...
    val entries = listOf(
        Triple("SETTINGS", Icons.Outlined.Settings, onSettings),
        Triple("PHONE", Icons.Outlined.Call, onPhone),
        Triple("MESSAGES", Icons.Outlined.Message, onMessages),
        Triple("CONTACTS", Icons.Outlined.Contacts, onPhone),
        Triple("WALLPAPER", Icons.Outlined.Wallpaper, onWallpaper),
        Triple("SYSTEM", Icons.Outlined.Android, onSettings)
    )
    CategoryPanel(modifier, "SYSTEM", ink, Icons.Outlined.Settings) {
        entries.forEach { (label, icon, action) -> MiniApp(label, icon, ink, action) }
    }
}
    ...
    Surface(modifier = modifier, color = L1voPanel.copy(alpha = .96f), shape = RoundedCornerShape(26.dp), shadowElevation = 5.dp) {
        Column(Modifier.padding(14.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(title, color = ink, fontWeight = FontWeight.Bold, style = MaterialTheme.typography.titleSmall)
            }
...
private fun MiniApp(label: String, icon: ImageVector, ink: Color, onClick: () -> Unit) {
    Surface(onClick = onClick, color = Color.White.copy(alpha = .72f), shape = RoundedCornerShape(16.dp), modifier = Modifier.fillMaxWidth().height(48.dp)) {
        Row(Modifier.fillMaxSize().padding(horizontal = 10.dp), verticalAlignment = Alignment.CenterVertically) {
            Icon(icon, label, tint = L1voDeepGreen, modifier = Modifier.size(20.dp))
            Spacer(Modifier.width(9.dp))
...
private fun SectionHeading(title: String, subtitle: String, ink: Color) {
    Column(Modifier.fillMaxWidth().padding(top = 2.dp)) {
        ...
        Text(subtitle, color = ink.copy(alpha = .78f), fontWeight = FontWeight.Medium, style = MaterialTheme.typography.bodySmall)
    }
}

@Composable
private fun FavoriteSlot(app: LaunchableApp?, ink: Color, onClick: (() -> Unit)?) {
    Surface(onClick = { onClick?.invoke() }, enabled = app != null, shape = RoundedCornerShape(18.dp), color = Color.White.copy(alpha = if (app != null) .90f else .52f), shadowElevation = if (app != null) 2.dp else 0.dp, modifier = Modifier.fillMaxWidth().height(86.dp)) {
        Column(Modifier.fillMaxSize().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
            if (app != null) {
                Image(app.icon.asImageBitmap(), app.label, Modifier.size(38.dp), contentScale = ContentScale.Fit)
                Spacer(Modifier.height(5.dp))
                Text(app.label, color = ink, fontWeight = FontWeight.Medium, style = MaterialTheme.typography.labelSmall, maxLines = 1)
            } else {
                Icon(Icons.Outlined.Add, "Empty favorite", tint = ink.copy(alpha = .45f), modifier = Modifier.size(24.dp))
...
private fun AppIcon(a: LaunchableApp, onOpen: (LaunchableApp) -> Unit, ink: Color) {
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
        ...
        Spacer(Modifier.height(5.dp))
        Text(a.label, color = ink, fontWeight = FontWeight.Medium, style = MaterialTheme.typography.labelMedium, maxLines = 1)
    }
}

private fun launchMessages(context: android.content.Context) {
    val intent = Intent(Intent.ACTION_MAIN).apply { addCategory(Intent.CATEGORY_APP_MESSAGING) }
    launch(context, intent)
}

private fun rememberAppUse(context: android.content.Context, packageName: String) {
    val prefs = context.getSharedPreferences(PREFS, android.content.Context.MODE_PRIVATE)
    val key = "app_use_$packageName"
    prefs.edit().putInt(key, prefs.getInt(key, 0) + 1).apply()
}

private fun loadFavoriteApps(context: android.content.Context, apps: List<LaunchableApp>): List<LaunchableApp> {
    val prefs = context.getSharedPreferences(PREFS, android.content.Context.MODE_PRIVATE)
...
fun L1voHub(ink: Color, onBack: () -> Unit, onSettings: () -> Unit, onLeau: () -> Unit, onWallpaper: () -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(horizontal = 20.dp, vertical = 18.dp)) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            IconButton(onClick = onBack) { Icon(Icons.AutoMirrored.Outlined.ArrowBack, "Back", tint = ink) }
            Column(Modifier.weight(1f)) {
                Text("L1VO", style = MaterialTheme.typography.headlineMedium, color = ink, fontWeight = FontWeight.SemiBold)
                Text("L1vo apps & features", color = L1voGreen, fontWeight = FontWeight.Medium)
            }
            IconButton(onClick = onWallpaper) { Icon(Icons.Outlined.Wallpaper, "Wallpaper", tint = L1voGreen) }
        }
        Spacer(Modifier.height(18.dp))
        Feature("L1vo Settings", "Native launcher settings", Icons.Outlined.Settings, ink, onSettings)
...
private fun Feature(t: String, s: String, i: ImageVector, ink: Color, onClick: () -> Unit) {
    Surface(onClick = onClick, color = L1voPanel.copy(alpha = .96f), shape = RoundedCornerShape(22.dp), modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp)) {
        Row(Modifier.padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
            Icon(i, t, tint = L1voDeepGreen, modifier = Modifier.size(28.dp))
            Spacer(Modifier.width(15.dp))
            Column {
                Text(t, color = ink, fontWeight = FontWeight.SemiBold, style = MaterialTheme.typography.bodyLarge)
                Text(s, color = ink.copy(alpha = .78f), fontWeight = FontWeight.Medium, style = MaterialTheme.typography.bodySmall)
            }
        }
    }
}
