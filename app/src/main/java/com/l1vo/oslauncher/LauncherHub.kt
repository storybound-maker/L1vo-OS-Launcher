...
fun AppHub(
    ...
    LazyVerticalGrid(
        ...
        item(span = { GridItemSpan(maxLineSpan) }) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                IconButton(onClick = onWallpaper) {
                    Icon(Icons.Outlined.Wallpaper, "Wallpaper", tint = L1voGreen)
                }
            }
        }

        item(span = { GridItemSpan(maxLineSpan) }) {
            SearchHub(query, { query = it }, ink)
        }
        ...
        if (query.isBlank()) {
            ...
            item(span = { GridItemSpan(maxLineSpan) }) {
                SectionHeading("FAVORITES", "Learns from the apps you use most", ink)
            }
            ...
            item(span = { GridItemSpan(maxLineSpan) }) {
                SectionHeading("ALL APPS", "Every launchable app on this device", ink)
            }
        } else {
            item(span = { GridItemSpan(maxLineSpan) }) {
                SectionHeading("SEARCH RESULTS", "Matching installed apps", ink)
            }
        }

        items(filteredApps, key = { it.packageName }) { app ->
            AppIcon(app, ::openApp, ink)
        }
    ...
    Surface(
        ...
        Row(
            ...
            BasicTextField(
                ...
                decorationBox = { inner ->
                    if (query.isEmpty()) {
                        Text("SEARCH HUB", color = ink.copy(alpha = .72f), fontWeight = FontWeight.SemiBold)
                    }
                    inner()
                }
            )
            if (query.isNotEmpty()) {
                IconButton(onClick = { onQueryChange("") }) {
                    Icon(Icons.Outlined.Close, "Clear search", tint = L1voDeepGreen)
                }
            }
        }
    }
...
private fun L1voAppsPanel(modifier: Modifier, ink: Color, onOpen: (String) -> Unit) {
    ...
    CategoryPanel(modifier, "L1VO APPS", ink, Icons.Outlined.Eco) {
        entries.forEach { (label, icon, key) ->
            MiniApp(label, icon, ink) { onOpen(key) }
        }
    }
}
...
private fun SystemAppsPanel(
    ...
    val entries = listOf(
        ...
        Triple("WALLPAPER", Icons.Outlined.Wallpaper, onSettings),
...
private fun SectionHeading(title: String, subtitle: String, ink: Color) {
    Column(Modifier.fillMaxWidth().padding(top = 2.dp)) {
        Text(title, color = ink, fontWeight = FontWeight.Bold, style = MaterialTheme.typography.titleMedium, letterSpacing = MaterialTheme.typography.titleMedium.letterSpacing)
...
private fun FavoriteSlot(app: LaunchableApp?, ink: Color, onClick: (() -> Unit)?) {
    Surface(
        ...
        color = Color.White.copy(alpha = if (app != null) .90f else .52f),
        shadowElevation = if (app != null) 2.dp else 0.dp,
