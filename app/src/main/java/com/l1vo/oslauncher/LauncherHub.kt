...
@Composable fun AppHub(apps:List<LaunchableApp>,ink:Color,onBack:()->Unit,onLeau:()->Unit,onWallpaper:()->Unit,onOpen:(LaunchableApp)->Unit,onL1vo:()->Unit){
    Column(Modifier.fillMaxSize().padding(horizontal=18.dp,vertical=16.dp)){
        ...
        LazyVerticalGrid(columns=GridCells.Fixed(4),modifier=Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(12.dp),horizontalArrangement=Arrangement.spacedBy(10.dp),contentPadding=PaddingValues(bottom=10.dp)){
            ...
            item{SystemCard("Settings",Icons.Outlined.Settings){launch(LocalContext.current,Intent(Settings.ACTION_SETTINGS))}}
            item{SystemCard("Phone",Icons.Outlined.Call){launch(LocalContext.current,Intent(Intent.ACTION_DIAL))}}
            item{SystemCard("Gallery",Icons.Outlined.Collections){launch(LocalContext.current,Intent(Intent.ACTION_VIEW).apply{type="image/*"})}}
        ...
        Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.End,verticalAlignment=Alignment.CenterVertically){Surface(onClick=onLeau,shape=RoundedCornerShape(24.dp),color=L1voInk,shadowElevation=6.dp){Row(Modifier.padding(horizontal=16.dp,vertical=11.dp),verticalAlignment=Alignment.CenterVertically){Text("Leau",color=Color.White,fontWeight=FontWeight.Medium);Spacer(Modifier.width(8.dp));Text("⌒  ⌒",color=Color.White,fontWeight=FontWeight.Bold)}}}
    }
}
