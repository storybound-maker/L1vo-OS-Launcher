...
fun AppHub(apps: List<LaunchableApp>, ink: Color, onBack: () -> Unit, onLeau: () -> Unit, onWallpaper: () -> Unit, onOpen: (LaunchableApp) -> Unit, onL1vo: () -> Unit, onLeacher: () -> Unit) {
    ...
    LazyVerticalGrid(columns = GridCells.Fixed(4), modifier = Modifier.fillMaxSize(), contentPadding = PaddingValues(start = 18.dp, end = 18.dp, top = 16.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(14.dp), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        item(span = { GridItemSpan(maxLineSpan) }) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) { Surface(onClick = onLeau, shape = RoundedCornerShape(50), color = L1voInk, shadowElevation = 6.dp) { Row(Modifier.padding(horizontal = 20.dp, vertical = 12.dp), verticalAlignment = Alignment.CenterVertically) { Icon(Icons.Outlined.Eco, "Leau", tint = Color.White, modifier = Modifier.size(20.dp)); Spacer(Modifier.width(8.dp)); Text("LEAU", color = Color.White, fontWeight = FontWeight.Bold) } } } }
    }
}

@Composable private fun SearchHub(query: String, onQueryChange: (String) -> Unit, ink: Color) {
    Surface(shape = RoundedCornerShape(50), color = Color.White.copy(alpha = .92f), shadowElevation = 4.dp, modifier = Modifier.fillMaxWidth().height(58.dp)) {
...
@Composable private fun FavoriteSlot(app: LaunchableApp?, ink: Color, onClick: () -> Unit) { Surface(onClick = onClick, enabled = app != null, shape = RoundedCornerShape(18.dp), color = Color.White.copy(alpha = if (app != null) .90f else .52f), shadowElevation = if (app != null) 2.dp else 0.dp, modifier = Modifier.fillMaxWidth().height(86.dp)) { Column(Modifier.fillMaxSize().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) { if (app != null) { Image(app.icon.asImageBitmap(), app.label, Modifier.size(38.dp), contentScale = ContentScale.Fit); Spacer(Modifier.height(5.dp)); Text(app.label, color = ink, fontWeight = FontWeight.Medium, style = MaterialTheme.typography.labelSmall, maxLines = 1) } else { Icon(Icons.Outlined.Add, "Empty favorite", tint = ink.copy(alpha = .45f), modifier = Modifier.size(24.dp)); Spacer(Modifier.height(4.dp)); Text("EMPTY", color = ink.copy(alpha = .55f), fontWeight = FontWeight.Medium, style = MaterialTheme.typography.labelSmall) } } } }

@Composable private fun AppIcon(a: LaunchableApp, onOpen: (LaunchableApp) -> Unit, ink: Color) { Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) { Surface(onClick = { onOpen(a) }, shape = RoundedCornerShape(18.dp), color = Color.White.copy(alpha = .90f), shadowElevation = 2.dp, modifier = Modifier.size(62.dp)) { Image(a.icon.asImageBitmap(), a.label, Modifier.padding(9.dp), contentScale = ContentScale.Fit) }; Spacer(Modifier.height(5.dp)); Text(a.label, color = ink, fontWeight = FontWeight.Medium, style = MaterialTheme.typography.labelMedium, maxLines = 1) } }
